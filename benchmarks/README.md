...
We welcome contributions of new problems from the community! See [this page](https://openenergybenchmark.org/key-insights#what-benchmark-problems-do-we-have-and-what-are-missing) for details about our current problem set, and what gaps we have that we would love your help to fill.
...
1. The PyPSA problems in `benchmarks/pypsa/` can be generated by using the Dockerfile present in that directory. Please see the [instructions](benchmarks/pypsa/README.md) for more details.

1. The JuMP-HiGHS problems in `benchmarks/jump_highs_platform/` contain only the metadata for the problems that are present in https://github.com/jump-dev/open-energy-modeling-benchmarks/tree/main/instances. These are fetched automatically by the benchmark runner from GitHub.

1. The metadata of all problems under `benchmarks/` are collected by the following script to generate a unified `results/metadata.yaml` file, when run as follows (using the `benchmarks` [pixi](https://pixi.sh) environment defined in the root `pixi.toml`):
   ```shell
