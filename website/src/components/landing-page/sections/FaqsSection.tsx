      ...
      >
        ...
        <div>
          <div className="mt-4 flex flex-col gap-6">
            ...
            />
            <FAQItem
              question="What do we mean by modelling framework, scenario, resolution, problem/instance, etc?"
              answer="We use the following nomenclature on this platform: An energy modelling framework, e.g. PyPSA or TIMES, is a software system that allows one to input country or region-specific data and model the energy system of interest. An energy model, e.g. eTIMES-EU or TIMES-NZ, is an instantiation of a modelling framework for a particular application or study. A scenario  is a specific case defined within a model framework. It specifies the system under study and its assumptions, such as geographic scope, temporal scope, sectoral scope, available technologies, technology costs, policy constraints, and other boundary conditions.
                      A scenario may be provided according to different resolutions: the resolution describes the spatial and temporal granularity used in the scenario. It defines, for example, the number of regions or nodes represented and the duration of the time step used in the optimization problem.
                      Each scenario-resolution couple defines a problem, captured as an LP or MPS file that is given as input to an optimization solver. In order to study the scaling behavior of solvers, and to provide solver developers with smaller versions of problems of interest, we group scenarios obtained by varying size parameters such as spatial or temporal resolution as being size instances of the same scenario. The full collection of problems on our platform represents the benchmark set."
            />
          </div>
        </div>
      </div>
