...
def bucket_problems_by_timeout(
    problems: dict[str, Any],
) -> tuple[set[str], set[str], set[str]]:
    """
    Split problem IDs into short-timeout, long-timeout, and skipped sets.

    Parameters
    ----------
    problems : dict
        The "problems" map from a parsed metadata YAML file.

    Returns
    -------
    tuple[set[str], set[str], set[str]]
        (short_timeout_ids, long_timeout_ids, skipped_ids). "S"/"M" sized
        problems are short-timeout, "L" sized problems are long-timeout;
        a problem with a "Skip because" value is added to skipped_ids
        regardless of size.

    Raises
    ------
    ValueError
        If a problem has an unrecognized "Size" value.
    """
    ...
    for problem_id, problem in problems.items():
        if problem["Size"] == "L":
            long_timeout_ids.add(problem_id)
        elif problem["Size"] in ["S", "M"]:
            short_timeout_ids.add(problem_id)
        else:
            print(
                f"::warning file=tests/validate_results.py::ERROR: Unknown size {problem['Size']}"
            )
            raise ValueError(f"Unknown size {problem['Size']}")
        if problem.get("Skip because") is not None:
            skipped_ids.add(problem_id)
...
def expected_solver_count(problem_class: str, is_long_timeout: bool) -> int:
    ...
    is_long_timeout : bool
        Whether this problem is in the long-timeout ("L" sized) bucket.
    ...
    for (bench, size), group in data.groupby(["Benchmark", "Size"]):
        ...
        if problem_id in short_timeout_ids:
            ...
            assert len(solvers_present) == expected, (
                f"expected {expected}, found {len(solvers_present)} solvers in results for {problem_id} ({problem_class})"
            )
            if problem_class == "LP":
        ...
        elif problem_id in long_timeout_ids:
            problem_class = meta["problems"][problem_id].get("Problem class", "")
            ...
            print(
                f"::warning file=tests/validate_results.py::ERROR: Unknown benchmark {problem_id}"
            )
